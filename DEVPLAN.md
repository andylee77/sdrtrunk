...
| F.3 | Call Log Database (SQLite) | Persist completed call sessions + per-talker events to SQLite. Two-table schema (call_sessions + call_events). Write on CallSession COMPLETE — no debounce needed. Per-system DB files in `{root}/call_logs/`. | 🔴 P0 | ☑ Complete (010 Phase 2) |
| F.4 | Call Log UI, Playback & Events Cleanup | Calls tab UI (from 010) with flat per-talker rows, filter toolbar (Hide Encrypted/Data/Ignored), historical data loading from DB. Future: inline audio player, transcript display. Events tab unchanged. | 🔴 P0 | ☑ Calls tab + history complete (010 Phase 2). Playback/transcription deferred. |
